...
# Use TEMP_FOLDER environment variable or default to current script's directory
temp_folder = os.environ.get("TEMP_FOLDER", os.path.dirname(os.path.abspath(__file__)))
csv_file = os.path.join(temp_folder, "elhub_vestfold.csv")
...
def query_elhub(date):
    ...
    # Filter for municipalities in Vestfold (Knr 3900 to 4000)
    df_vestfold = df_combined[
        (df_combined["attributes.municipalityNumber"].astype(int) > 3900)
...
if __name__ == "__main__":
    ...
    main(start_date="2025-01-01", end_date="2025-12-31")
